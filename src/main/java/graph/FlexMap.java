...
public class FlexMap <K extends Cloneable, V extends Cloneable> implements Serializable, Map<K, V>, Cloneable {
    ...
    /**
     * Replaces every occurrence of oldKey by newKey
     * @param oldKey the key to be replace
     * @param newKey the key to use instead
     */
    public void replaceKey(K oldKey, K newKey) {
        Map<K,V> new_values = new LinkedHashMap<>();
        _values.forEach((k, v) -> {if(k.equals(oldKey)){new_values.put(newKey, v);}else{new_values.put(k,v);}});
        _values = new_values;
    }

    /**
     * swaps these two keys : iterating over all, each time one is encountered, the user is used instead
     * @param key1 a key
     * @param key2 another key
     */
    public void swapKeys(K key1, K key2) {
        Map<K,V> new_values = new LinkedHashMap<>();
        _values.forEach((k, v) -> {if(k.equals(key1)){new_values.put(key2, v);}else if(k.equals(key2)){new_values.put(key1, v);}else{new_values.put(k,v);}});
        _values = new_values;
    }
    ...
    public boolean replace(K key, V oldValue, V newValue) {
        return _values.replace(key, oldValue, newValue);
    }
    ...
    public V replace(K key, V value) {
        return _values.replace(key, value);
    }
    ...
    public V computeIfAbsent(K key, Function<? super K, ? extends V> mappingFunction) {
        return _values.computeIfAbsent(key, mappingFunction);
    }
    ...
    public V computeIfPresent(K key, BiFunction<? super K, ? super V, ? extends V> remappingFunction) {
        return _values.computeIfPresent(key, remappingFunction);
    }
    ...
    public V compute(K key, BiFunction<? super K, ? super V, ? extends V> remappingFunction) {
        return _values.compute(key, remappingFunction);
    }
    ...
    public V merge(K key, V value, BiFunction<? super V, ? super V, ? extends V> remappingFunction) {
        return _values.merge(key, value, remappingFunction);
    }
}